...
Namespace CustomFontEditControl
    ...
    Friend Class RepositoryItemCustomFontEdit
        ...
        Shared Sub New()
            Call RegisterCustomFontEdit()
        End Sub

        ' static register method
        Public Shared Sub RegisterCustomFontEdit()
            Call EditorRegistrationInfo.Default.Editors.Add(New EditorClassInfo(CustomFontEditorName, GetType(CustomFontEdit), GetType(RepositoryItemCustomFontEdit), GetType(ComboBoxViewInfo), New ButtonEditPainter(), True, CType(Nothing, System.Drawing.Image)))
        End Sub
        ...
        Friend Shared Sub DrawFontName(ByVal gr As Graphics, ByVal name As String, ByVal font As Font, ByVal normalFont As Font, ByVal brush As Brush, ByVal bounds As Rectangle, ByVal showPreview As Boolean)
            Using sf As StringFormat = New StringFormat()
                ...
                If font.Height < bounds.Height OrElse font.Height \ bounds.Height > 2 Then sf.LineAlignment = StringAlignment.Center
        ...
        Friend Shared Overloads Sub DrawFontBoxItem(ByVal e As ListBoxDrawItemEventArgs, ByVal item As RepositoryItemCustomFontEdit, ByVal IsDrawStyle As Boolean, ByVal sCurrentFontName As String)
            If Equals(sCurrentFontName, String.Empty) Then Return
            Using foreBrush As SolidBrush = New SolidBrush(e.Appearance.ForeColor), backBrush As SolidBrush = New SolidBrush(e.Appearance.BackColor)
                If Not FontItemPaintHelper.DrawItemBar(e, TryCast(item, RepositoryItemComboBox)) Then e.Graphics.FillRectangle(backBrush, e.Bounds)
                Using family As FontFamily = New FontFamily(sCurrentFontName)
                    ...
                    Using font As Font = New Font(family, e.Appearance.Font.Size, currentStyle)
                        DrawFontName(e.Graphics, e.Item.ToString(), font, e.Appearance.Font, foreBrush, e.Bounds, True)
                        e.Handled = True
                    End Using
                End Using
            End Using
